...
export default function PasswordConfirmation({ onNavigate }: { onNavigate?: (view: string) => void }) {
  return (
      ...
      <div className="w-full max-w-[1000px] flex flex-col items-center text-center gap-8 animate-in fade-in zoom-in-95 duration-500">
        <div className="flex flex-col gap-4 items-center">
          ...
          <span className="text-gray-400 text-base md:text-lg leading-relaxed max-w-[600px]">
            Please check your inbox and follow the instructions to reset your password. Didn't receive the email? Resend or check your spam folder
          </span>
        </div>
        ...
        <div className="flex flex-col gap-4 items-center">
          ...
          {/* Simulation Link for Prototype Flow */}
          <button
            onClick={() => onNavigate?.('rewrite-password')}
            className="text-gray-500 hover:text-white text-sm underline cursor-pointer bg-transparent border-none mt-4"
          >
            (Prototype: Simulate clicking email link)
          </button>
        </div>
      </div>
    </div>
